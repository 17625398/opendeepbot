...
| `deepseek_code_edit` | Apply code patches in diff format (DeepSeek V4) |
| `lsp_diagnostic`    | Post-edit LSP diagnostics (pyright/rust-analyzer) |
...
| `deep_coding`    | plan → code → review → test (DeepSeek V4) |
...
| `deeptutor/core/stream.py`          | StreamEvent protocol (+ REASONING_CONTENT, COST_UPDATE) |
...
| `deeptutor/services/llm/auto_mode.py` | Auto mode (model+reasoning router) |
| `deeptutor/tools/builtin/code_edit_tool.py` | DeepSeek code edit tool |
| `deeptutor/tools/builtin/lsp_tool.py` | LSP diagnostic tool |
| `deeptutor/capabilities/deep_coding.py` | DeepCoding capability (plan→code→review→test) |
