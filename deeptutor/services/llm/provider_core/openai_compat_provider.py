...
class OpenAICompatProvider(LLMProvider):
    ...
    @classmethod
    def _extract_usage(cls, response: Any) -> dict[str, int | float]:
        usage_obj = None
        ...
        if usage_map is not None:
            ...
            # DeepSeek V4 cache fields
            cache_hit = usage_map.get("prompt_cache_hit_tokens")
            if cache_hit is not None:
                usage["prompt_cache_hit_tokens"] = int(cache_hit)
            cache_miss = usage_map.get("prompt_cache_miss_tokens")
            if cache_miss is not None:
                usage["prompt_cache_miss_tokens"] = int(cache_miss)
        elif usage_obj:
            usage = {
            ...
            if hasattr(usage_obj, "prompt_cache_hit_tokens"):
                usage["prompt_cache_hit_tokens"] = int(usage_obj.prompt_cache_hit_tokens)
            if hasattr(usage_obj, "prompt_cache_miss_tokens"):
                usage["prompt_cache_miss_tokens"] = int(usage_obj.prompt_cache_miss_tokens)
        return usage

    # DeepSeek V4 pricing (as of 2026-05-07, 75% discount valid until 2026-05-31)
    _DEEPSEEK_PRICING = {
        "deepseek-v4-pro": {
        ...
        "deepseek-chat": {  # Legacy alias for flash
            "cache_hit": 0.0028,
            "cache_miss": 0.14,
            "output": 0.28,
        },
        "deepseek-reasoner": {  # Legacy alias for flash
            "cache_hit": 0.0028,
            "cache_miss": 0.14,
            "output": 0.28,
        },
    ...
    @classmethod
    def _calculate_cost(cls, usage: dict[str, int], model: str) -> dict[str, float]:
        ...
        pricing = cls._DEEPSEEK_PRICING.get(model_key)
        if pricing is None:
            # Try to find a partial match
            for key in cls._DEEPSEEK_PRICING:
                if key in model_key:
                    pricing = cls._DEEPSEEK_PRICING[key]
                    break
        ...
        # If cache fields not present, estimate from prompt_tokens
        if cache_hit == 0 and cache_miss == 0:
            # Assume no cache hit for simplicity
            cache_miss = prompt_tokens
        
        # Calculate costs
        hit_cost = (cache_hit / 1_000_000) * pricing["cache_hit"]
        miss_cost = (cache_miss / 1_000_000) * pricing["cache_miss"]
        output_cost = (completion_tokens / 1_000_000) * pricing["output"]
        total_cost = hit_cost + miss_cost + output_cost
        
        return {
            "total_cost": round(total_cost, 6),
            "prompt_cache_hit_cost": round(hit_cost, 6),
            "prompt_cache_miss_cost": round(miss_cost, 6),
    ...
    @classmethod
    def _parse_chunks(cls, chunks: list[Any], model: str | None = None) -> LLMResponse:
    ...
    async def chat(
        ...
        **extra_kwargs: Any,
        ...
        try:
            ...
            try:
                return self._parse(await self._client.chat.completions.create(**request_kwargs), model=model)
                ...
                if request_kwargs.get(
                    ...
                    return self._parse(await self._client.chat.completions.create(**retry_kwargs), model=model)
                raise
    ...
    async def chat_stream(
        ...
        **extra_kwargs: Any,
        ...
        try:
            ...
            return self._parse_chunks(chunks, model=model)
