...
from .auto_mode import route_turn
...
async def complete(
    ...
    **kwargs: Any,
    ...
    # Auto mode: route to select model and reasoning effort
    if config.model == "auto":
        resolved_model, resolved_reasoning = await route_turn(
            prompt=prompt,
            context_messages=messages,
            config=config,
        )
        config.model = resolved_model
        config.reasoning_effort = resolved_reasoning
...
async def stream(
    ...
    **kwargs: Any,
    ...
    async def _on_reasoning_delta(chunk: str) -> None:
        ...
        # Send REASONING_CONTENT event to StreamBus if available
        if stream_bus is not None:
            from deeptutor.core.stream import StreamEventType
            await stream_bus.emit(
                StreamEvent(type=StreamEventType.REASONING_CONTENT, content=chunk, source="llm")
            )
